"""Generate presentation-quality figures for the pitch deck from real outputs.

Run: /opt/anaconda3/bin/python src/make_figures.py
Outputs -> outputs/figures/ppt/*.png
"""
...
BLUE, YELLOW, INK = "#2874F0", "#FFE11B", "#1a1a2e"
TIER = {"Low": "#27ae60", "Medium": "#f39c12", "High": "#e67e22", "Critical": "#c0392b"}
...
colors = [YELLOW if h == 2 else BLUE for h in by_hr.index]
plt.bar(by_hr.index, by_hr.values, color=colors, edgecolor="white")
plt.annotate("Peak at 2 AM — freight breakdowns,\nNOT rush hour", xy=(2, by_hr.max()),
             xytext=(6, by_hr.max()*0.92), fontsize=12, color=INK,
...
# 2) Mean EIS by cause — face validity of the Impact Score
...
# 3) Road-closure model leaderboard (k-fold PR-AUC)
cv = pd.read_csv(C.METRIC_DIR / "closure_cv.csv").sort_values("PR_AUC")
plt.figure(figsize=(8.5, 5))
bars = plt.barh(cv["model"], cv["PR_AUC"],
                color=[YELLOW if m == "STACK_logit" else BLUE for m in cv["model"]])
plt.axvline(0.083, color=TIER["Critical"], ls="--", lw=2)
plt.text(0.083, 0.3, " prevalence\n baseline 0.083", color=TIER["Critical"], fontsize=10)
plt.xlabel("PR-AUC (5-fold CV)"); plt.title("Road-closure: 10-model zoo + stacked ensemble")
save("closure_leaderboard.png")

# 4) Daily volume with surge days annotated — why forecasting matters
daily = df.assign(d=t.dt.date).groupby("d").size()
plt.figure(figsize=(9.5, 4.4))
plt.plot(daily.index, daily.values, color=BLUE, lw=1.6)
plt.axhline(daily.median(), color="grey", ls=":", label=f"median {daily.median():.0f}/day")
for d, v in daily.sort_values(ascending=False).head(2).items():
    plt.annotate(f"{v} events", xy=(d, v), xytext=(d, v+12), fontsize=11, color=TIER["Critical"],
                 ha="center", arrowprops=dict(arrowstyle="->", color=TIER["Critical"]))
plt.ylabel("Events per day"); plt.title("Demand spikes are real — and forecastable")
plt.legend(); save("daily_volume.png")

# 5) Learning-loop model bias by cause (L4 drift)
